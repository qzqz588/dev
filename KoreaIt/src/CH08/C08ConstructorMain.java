package CH08;
//생성자 메서드
//객체 생성시 필요한 메모리 공간형성과 초기값 부여에 사용되는 메서드
//객체 생성시 한번 호출되는 메서드(재사용불가)
//생성자 메서드의 이름은 클래스의 이름과 동일하게 작성하고 반환 자료형은 작성하지 않는다.
//생성자 매서드를 클래스 내에 작성하지 않으면 칸파일러에 의해 주입되는 생성자가 있는데
//이를 디폴트 생성자라고 한다.
